...
class Player():
    ...
    def start(self):
        try:
            match self.fishing_strategy:
                case 'spin':
                    self.do_spin_fishing()
                case 'bottom':
                    self.do_bottom_fishing()
                case 'strong_pirking':
                    self.do_pirking_fishing(duration=1.75, delay=4)
                case 'pirking':
                    self.do_pirking_fishing(duration=0.5, delay=2)
                case 'spin_with_pause':
                    self.do_spin_fishing_with_pause(duration=self.duration, delay=self.delay)
                #todo: default
    ...
    def do_spin_fishing_with_pause(self, duration, delay):
        while True:
            #todo: use another thread to monitor it
            if is_tackle_broked():
                self.save_screenshot()
            ...
            self.start_retrieving_stage(duration=8) # default
            # skip pulling if there is no fish
            if not is_fish_hooked():
                self.cast_miss_count += 1
                continue
    ...
    def do_bottom_fishing(self, retrieval_duration=4, retrieval_delay=2):
        ...
        while True:
            ...
            self.start_retrieving_stage(duration=retrieval_duration, delay=retrieval_delay)
            if is_fish_hooked():
                self.start_pulling_stage()
            self.start_resetting_stage()
            self.tackle.cast(power_level=2, cast_delay=4)
