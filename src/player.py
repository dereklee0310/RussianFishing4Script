...
class Player:
    ...
    def __init__(
        self, args: Namespace, config: ConfigParser, profile_name: str
    ) -> None:
    ...
    def _build_game_config(self, config: ConfigParser) -> None:
        ...
        self.unmarked_release_whitelist = [
            key.strip()
            for key in game_section.get("unmarked_release_whitelist").split(",")
        ]
    ...
    def bottom_fishing(self) -> None:
        ...
        rod_idx = -1
        bottom_rods = [
            key.strip() for key in self.shortcut_dict["bottom_rods"].split(",")
        ]
    ...
    def marine_fishing(self) -> None:
        ...
        while True:
            ...
            self.retrieving_stage(
                duration=2.2, delay=0, accelerated=True
            )  # at least 2.2
            if monitor.is_fish_hooked():
                self.drinking_stage()
                self.pulling_stage()
    ...
    def drinking_stage(self) -> None:
        ...
        cur_time = time()
        if (
            not self.alcohol_drinking_enabled
            or cur_time - self.pre_alcohol_drink_time < self.alcohol_drinking_delay
        ):
            return
    ...
    def retrieving_stage(self, duration=16, delay=4, accelerated=False):
        ...
        while not self.tackle.retrieve(duration, delay, lift_enabled=self.lift_enabled):
            ...
            if (
                not monitor.is_energy_high(threshold=0.9)
                and self.coffee_drinking_enabled
            ):
