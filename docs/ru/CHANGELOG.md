...
### 🆕 Новые функции
- **Локализация интерфейса**: добавлена полная поддержка русского языка для всего интерфейса скрипта — меню, логи, результаты, уведомления и справка по аргументам. Язык определяется параметром `LANGUAGE` в `config.yaml` (`"en"` или `"ru"`). Файлы переводов хранятся в `rf4s/i18n/locales/`.
